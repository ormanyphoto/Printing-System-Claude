    ...
    <div className="min-h-screen bg-background">
      ...
      <header className="sticky top-0 z-30 bg-background/95 backdrop-blur-md border-b border-border">
        <div className="flex items-center justify-between px-5 lg:px-8 h-14">
          <button
            onClick={() => navigate("/")}
            className="flex items-center gap-2.5 font-body text-sm text-muted-foreground hover:text-foreground transition-colors"
          >
            <ArrowLeft className="w-4 h-4" />
            <span className="hidden sm:inline font-body text-xs uppercase tracking-wider font-semibold">{t("order.back", "Back")}</span>
          </button>
          <span className="font-display text-lg tracking-tight text-foreground font-semibold">
            ChromaLuxe
          </span>
          <div className="w-16" />
        </div>
        ...
        <div className="w-full">
          <div className="mx-auto">
            <div className="flex flex-col lg:flex-row lg:items-start gap-0" style={{ direction: "ltr" }}>
              ...
              {showPreview && (
                <div className="hidden lg:flex lg:flex-col lg:w-2/3 lg:self-start lg:sticky lg:top-[88px] bg-muted/20" style={{ direction: i18n.language === "he" ? "rtl" : "ltr", height: "calc(100vh - 88px)" }}>
              ...
              <div className={`${showPreview ? "lg:w-1/3 lg:border-s lg:border-border" : "w-full max-w-2xl mx-auto"} transition-all lg:flex lg:flex-col`} style={{ direction: i18n.language === "he" ? "rtl" : "ltr" }}>

                <div className="lg:overflow-y-auto lg:flex-1 lg:min-h-0 px-5 py-5 lg:px-8 space-y-1" style={{ maxHeight: showPreview ? "calc(100vh - 88px)" : undefined }}>
                   ...
                   <section>
                    {step2Done && uploadedImage ? (
                      <>
                        ...
                        <div className="lg:hidden flex items-center gap-3 p-3 border border-border/60 rounded-lg bg-card mb-2">
                          <img src={previewSrc} alt="" className="w-10 h-10 object-cover rounded-md" />
                          <span className="flex-1 font-body text-sm text-foreground truncate">{t("order.step1")}</span>
                          <button onClick={() => setShowEditor(true)} className="font-body text-xs text-accent font-semibold px-2">{t("order.editPhoto")}</button>
                          <button onClick={() => { clearImage(); setSelectedSize(null); }} className="font-body text-xs text-muted-foreground px-2 hover:text-destructive transition-colors">
                        ...
                        <div className="hidden lg:block border-b border-border/60">
                          <div className="flex items-center gap-3 px-2 py-4">
                            <span className="flex-shrink-0 w-9 h-9 rounded-full bg-accent/10 flex items-center justify-center text-accent">
                              <ImagePlus className="w-5 h-5" strokeWidth={1.5} />
                            </span>
                            <div className="flex-1 min-w-0">
                              <p className="font-body text-sm font-semibold text-foreground">{t("order.step1", "Upload Photo")}</p>
                              <p className="font-body text-xs text-muted-foreground truncate mt-0.5">{uploadedImage?.file?.name || ""}</p>
                            </div>
                ...
                {priceBreakdown && selectedSize && selectedProduct && (
                  <div className="sticky bottom-0 bg-card/95 backdrop-blur-md border-t border-border shadow-[0_-4px_20px_rgba(0,0,0,0.06)] px-5 lg:px-8 py-4 space-y-3 z-10">
                    ...
                    <div className="flex gap-3">
                      {!isEmbed && (
                        <Button variant="outline" className="flex-shrink-0 h-12 font-body text-sm font-semibold" disabled={savingDraft} onClick={handleSaveDraft}>
                          {savingDraft ? t("draft.saving") : t("draft.save")}
                        </Button>
                      )}
                      <Button className="flex-1 h-12 text-sm font-body font-bold tracking-wide uppercase bg-accent text-accent-foreground hover:bg-accent/90" disabled={addingToCart} onClick={handleAddToCart}>
...
function OptionRow({ icon, label, value, onClick, expandable, expandedContent, expanded, onToggle, done }: {
  ...
  return (
    <div className="border-b border-border/60">
      <button
        onClick={handleClick}
        className="w-full flex items-center gap-3 px-2 py-4 text-left group rounded-md hover:bg-muted/30 transition-colors"
      >
        <span className={`flex-shrink-0 w-9 h-9 rounded-full flex items-center justify-center transition-colors ${done ? "bg-accent/10 text-accent" : "bg-muted/50 text-muted-foreground"}`}>{icon}</span>
        <div className="flex-1 min-w-0">
          <p className="font-body text-sm font-semibold text-foreground">{label}</p>
          {value && <p className="font-body text-xs text-muted-foreground truncate mt-0.5">{value}</p>}
        </div>
        {done && !expanded && (
          <span className="flex-shrink-0 w-5 h-5 rounded-full bg-accent flex items-center justify-center">
            <svg className="w-3 h-3 text-accent-foreground" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={3}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
            </svg>
          </span>
        )}
        <svg className={`w-4 h-4 text-muted-foreground/60 flex-shrink-0 transition-transform duration-200 ${expanded ? "rotate-90" : ""} ${document.documentElement.dir === "rtl" && !expanded ? "rotate-180" : ""}`} fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2.5}>
          <path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7" />
        </svg>
      </button>
      {expandable && expanded && (
        <div className="px-2 pb-5 pt-1 animate-fade-in">
          {expandedContent}
        </div>
      )}
    </div>
